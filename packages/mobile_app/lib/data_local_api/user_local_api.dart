...
class UserLocalApi with HasLocalApis {
  ...
  Future<void> saveUser({required final UserModel user}) =>
      localDb.setMap(key: _persistenceKey, value: user.toJson());

  Future<UserModel> loadUser() => localDb.getItem(
    key: _persistenceKey,
    defaultValue: UserModel.empty,
    fromJson: (final json) => json.isEmpty ? null : UserModel.fromJson(json),
  );
}
